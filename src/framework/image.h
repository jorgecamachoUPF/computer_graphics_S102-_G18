...
public:
  ...
  Color &GetPixelRef(unsigned int x, unsigned int y) {
    return pixels[y * width + x];
  }
  ...
  Image GetArea(unsigned int start_x, unsigned int start_y, unsigned int width,
                unsigned int height);
  ...
  // }); or callback sintax:   img.forEachPixel( mycallback ); //the callback
  // has to be Color mycallback(Color c) { ... }
  template <typename F> Image &ForEachPixel(F callback) {
    for (unsigned int pos = 0; pos < width * height; ++pos)
...
public:
  ...
  float &GetPixelRef(unsigned int x, unsigned int y) {
    return pixels[y * width + x];
  }
